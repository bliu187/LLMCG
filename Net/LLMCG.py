...
from Net import Entity_Aware_Embedding, PCNN, GNN
import numpy as np

class LLMCG(nn.Module):
    def __init__(self, word_vec_dir, edge, constraint, type_num, rel_num, opt):
        super(LLMCG, self).__init__()
        ...
        self.graph_encoder = GNN(edge, type_num, rel_num, opt['graph_emb'], opt['graph_hid'], graph_out,
                                 encoder=opt['graph_encoder'], num_layers=opt['num_layers'], num_heads=opt['num_heads'], data_set=opt['dataset'])
